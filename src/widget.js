...
function start() {
  ...
  for (let i = 0; i < widgets.length; i++) {
    ...
    const username = parentNode.dataset.username;
...
function lastPushedDay(repos) {
  ...
  for (let i = 0; i < repos.length; i++) {
    const pushedDate = new Date(repos[i].pushed_at);
...
function updateRepoDetails(repos, widgetId) {
  for (let i = 0; i < repos.length; i++) {
    const language = repos[i].language ? repos[i].language : 'Unknown';
